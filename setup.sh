...
# UAV Data Collector Setup Script
echo "Setting up UAV Data Collector..."
...
sudo apt install -y python3-pip python3-dev git python3-opencv python3-picamera

# Create project directory
echo "Creating project directory..."
mkdir -p ~/uav_data_collector/buffer/sensor_readings
mkdir -p ~/uav_data_collector/buffer/images
mkdir -p ~/uav_data_collector/buffer/system_status
mkdir -p ~/uav_data_collector/logs

# Install Python dependencies
echo "Installing Python dependencies..."
pip3 install pymongo dnspython python-dotenv opencv-python numpy pillow schedule

# Create .env file
echo "Creating .env file..."
cat > ~/uav_data_collector/.env << EOL
MONGODB_URI=${MONGODB_URI:-mongodb://localhost:27017/vayu-uav}
MONGODB_DB=${MONGODB_DB:-vayu-uav}
MISSION_ID=${MISSION_ID:-mission_1}
DEVICE_ID=${DEVICE_ID:-raspberry_pi_1}
EOL

# Copy the data collector script
echo "Copying data collector script..."
cat > ~/uav_data_collector/uav_data_collector.py << 'EOL'
#!/usr/bin/env python3
...
def capture_and_detect():
    try:
        ...
        #  you would capture an image from the camera
        # For example, using PiCamera:
...
sudo tee /etc/systemd/system/uav-data-collector.service > /dev/null << EOL
[Unit]
Description=UAV Data Collector Service
After=network.target

[Service]
User=pi
WorkingDirectory=/home/pi/uav_data_collector
ExecStart=/usr/bin/python3 /home/pi/uav_data_collector/uav_data_collector.py
Restart=always
RestartSec=10
...
chmod +x ~/uav_data_collector/uav_data_collector.py

# Enable and start the service
echo "Enabling and starting the service..."
sudo systemctl enable uav-data-collector.service
sudo systemctl start uav-data-collector.service

echo "UAV Data Collector setup complete!"
echo "Service status:"
sudo systemctl status uav-data-collector.service

echo "To view logs, run: sudo journalctl -u uav-data-collector.service -f"
echo "To manually start the collector: python3 ~/uav_data_collector/uav_data_collector.py"
